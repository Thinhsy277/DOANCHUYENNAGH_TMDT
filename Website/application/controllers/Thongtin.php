...
class Thongtin extends CI_Controller {
    ...
    function __construct() {
        ...
        $sessionCustomer = $this->session->userdata('sessionKhachHang');
        $customerId = isset($sessionCustomer['id']) ? $sessionCustomer['id'] : $this->session->userdata('id');
        $this->customerId = $customerId;
        $info=$this->Minfocustomer->customer_detail_id($customerId);
        if(empty($info) && !empty($sessionCustomer)){
            $info = [
                'id' => $customerId,
                'username' => isset($sessionCustomer['username']) ? $sessionCustomer['username'] : '',
                'fullname' => isset($sessionCustomer['fullname']) ? $sessionCustomer['fullname'] : '',
                'email' => isset($sessionCustomer['email']) ? $sessionCustomer['email'] : '',
                'phone' => isset($sessionCustomer['phone']) ? $sessionCustomer['phone'] : ''
            ];
        }
        $this->data['info']=$info;
