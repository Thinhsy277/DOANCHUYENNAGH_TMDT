...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $customer = $this->Mcustomer->customer_login($username, $password);
            if($customer != FALSE){
                $this->session->set_userdata('sessionKhachHang',$customer);
                $this->session->set_userdata('id',$customer['id']);
                $this->session->set_userdata('email',$customer['email']);
                $this->session->set_userdata('sessionKhachHang_name',$customer['fullname']);
                $this->session->set_flashdata('success_message', 'Đăng nhập thành công!');
                $this->session->set_flashdata('error_message', '');
                redirect('trang-chu','refresh');
            }else{
                $this->session->set_flashdata('error_message','Bạn đã nhập sai tài khoản hoặc mật khẩu');
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_user']    = 'hoahuongduong05124@gmail.com';
            $config['smtp_pass']    = 'ytotxwzbrwkoddjd';
            ...
            $this->email->from('hoahuongduong05124@gmail.com', 'Website');
            $this->email->to($email);
...
//laylaipass
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('Website', 'Website');
            $this->email->to($list['email']);
            $this->email->subject('Website - Lấy lại mật khẩu');
